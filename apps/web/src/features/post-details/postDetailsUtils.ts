...
export const resolveDisplayLocationLabel = ({
  ...
  const postArea = normalizeLocationValue(postAreaInput);
  const sellerAreaValue = normalizeLocationValue(sellerArea);
  const resolvedArea = postArea || sellerAreaValue;

  const postCityRaw = normalizeLocationValue(postLocation);
...
export const getDisplayImages = (post: Post): {
  ...
  const displayImages = postImages.length > 0 ? postImages : [""];
  return {
    displayImages,
