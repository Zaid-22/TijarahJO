...
export function MobilePhonesTabletsPage({
  ...
  const filteredProducts = useMemo(() => {
    const categoryProducts = products.filter(
      (p) =>
        p.category === "Mobile Phones & Tablets" &&
        p.status !== "SOLD" &&
        p.status !== "DELETED",
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div
          className={`grid ${viewMode === "grid-4" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-4" : viewMode === "grid-3" ? "grid-cols-1 sm:grid-cols-2 md:grid-cols-3" : viewMode === "grid-2" ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1"} gap-6`}
        >
          {displayedItems.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center py-16">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-center">
                {activeSearchQuery
                  ? language === "ar"
                    ? `لم نتمكن من العثور على "${activeSearchQuery}"`
                    : `No posts matching "${activeSearchQuery}"`
                  : language === "ar"
                    ? "لا توجد منشورات"
                    : "No posts available"}
              </p>
