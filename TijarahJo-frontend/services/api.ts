...
export const postsApi = {
  ...
  createPost: async (postData: CreatePostRequest): Promise<PostResponse> => {
    ...
    if (response.success && response.data) {
      ...
      enrichedPostData.Location = postData.location || postData.city || "Jordan";
  ...
  updatePost: async (postData: UpdatePostRequest): Promise<PostResponse> => {
    ...
    const backendPost = {
      ...
      CreatedAt: currentPost.CreatedAt || currentPost.createdAt || new Date().toISOString(),
      IsDeleted: currentPost.IsDeleted !== undefined ? currentPost.IsDeleted : false,
      City: postData.location || postData.city || currentPost.City || "Jordan",
