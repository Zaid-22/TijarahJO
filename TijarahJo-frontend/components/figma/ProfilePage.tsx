import { MapPin, Calendar, Package, Edit, Phone, Mail, Plus, Trash2, Eye, Settings } from "lucide-react";
...
import { SellItemDialogContent } from "./SellItemDialog";
...
export function ProfilePage({
  ...
  // 3. Compare seller with username (another fallback)
  const myProducts = products.filter(
    (p) =>
      p.sellerId === userProfile.id ||
      p.seller === currentUserName ||
      p.seller === userProfile.username ||
      p.seller === userProfile.name
  );
  
  // Separate into active and sold listings (don't show DELETED)
  const activeListings = myProducts.filter(p => p.status !== "SOLD" && p.status !== "DELETED");
            ...
            <div className="flex-1 text-white w-full sm:w-auto">
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-sm opacity-90 mb-3">
                <div className="flex items-center gap-1">
                  ...
                  <span>{userProfile.city || userProfile.location}{userProfile.area ? `, ${userProfile.area}` : ''}</span>
                </div>
                <div className="flex items-center gap-1">
