...
export function CategoryPage({ 
  ...
  const [sortBy, setSortBy] = useState<"newest" | "oldest" | "price-low" | "price-high" | "most-viewed" | "name-az" | "name-za">("newest");
  ...
  }, [products, activeSearchQuery, categoryName]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a]">
      <Header 
        ...
        onSearchSubmit={() => setActiveSearchQuery(searchQuery.trim())}
        onBack={onBack}
        onShowFavorites={onShowFavorites}
        onShowProfile={onShowProfile}
        onShowSettings={onShowSettings}
        onShowSellItem={onShowSellItem}
        onLogout={onLogout}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl text-gray-900 dark:text-white">
                {language === "ar" ? currentCategory?.nameAr : categoryName}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {language === "ar" 
                  ? `تصفح جميع ${currentCategory?.nameAr}` 
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between mb-6 gap-4">
          ...
          <div className="flex items-center gap-2 flex-1 sm:flex-initial">
            ...
            >
              ...
              <option value="most-viewed">{language === "ar" ? "الأكثر مشاهدة" : "Most Viewed"}</option>
              <option value="name-az">{language === "ar" ? "الاسم: أ-ي" : "Name: A-Z"}</option>
              <option value="name-za">{language === "ar" ? "الاسم: ي-أ" : "Name: Z-A"}</option>
            </select>
          </div>
