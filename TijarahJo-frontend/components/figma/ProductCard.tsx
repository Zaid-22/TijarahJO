...
export function ProductCard({ product, onProductClick, viewMode = "grid-4", isFavorite = false, onFavoriteToggle, isAuthenticated = true, currentUserName, hideCategoryBadge }: ProductCardProps) {
  ...
  const isOwner = currentUserName && product.seller === currentUserName;
  ...
  if (isListView) {
    ...
    return (
      <div 
        className="group bg-white dark:bg-gray-800/80 dark:border dark:border-gray-700 rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-strong animate-fade-in flex flex-col sm:flex-row backdrop-blur-sm"
        style={{ 
          boxShadow: "0 2px 8px rgba(0, 0, 0, 0.08)",
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.boxShadow = "0 12px 28px rgba(10, 74, 191, 0.15)";
        }}
        onMouseLeave={(e) => {
      ...
      >
        ...
        <div className="relative w-full sm:w-64 aspect-square overflow-hidden flex-shrink-0">
          ...
          {/* SOLD Badge */}
          {product.status === "SOLD" && (
            <div className={`absolute ${!hideCategoryBadge && isAuthenticated ? 'top-[4rem]' : 'top-3'} right-3 z-10`}>
              <Badge 
                className="backdrop-blur-md px-3 py-1 shadow-sm"
                style={{ 
        ...
        <div className="flex-1 p-6 flex flex-col justify-between">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 dark:text-gray-400 mb-3">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full flex items-center justify-center bg-gray-100 dark:bg-gray-700">
                      <MapPin className="w-3.5 h-3.5" style={{ color: "#0A4ABF" }} />
                    </div>
                    <span>{product.location}{product.area ? `, ${product.area}` : ''}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full flex items-center justify-center bg-gray-100 dark:bg-gray-700">
                      <User className="w-3.5 h-3.5" style={{ color: "#0A4ABF" }} />
                    </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <Button
              size="lg"
              className="hover:opacity-90 transition-all duration-300 shadow-md hover:shadow-lg px-8"
  ...
  return (
    <div 
      className="group bg-white dark:bg-gray-800/80 dark:border dark:border-gray-700 rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-strong animate-fade-in backdrop-blur-sm flex flex-col h-full"
      style={{ 
        boxShadow: "0 2px 8px rgba(0, 0, 0, 0.08)",
    ...
    >
      ...
      <div className="relative w-full overflow-hidden flex-shrink-0" style={{ aspectRatio: '4/3' }}>
        ...
        {product.status === "SOLD" && (
          <div className={`absolute ${!hideCategoryBadge && isAuthenticated ? 'top-[3.5rem]' : 'top-3'} right-3 z-10`}>
            <Badge 
              className="backdrop-blur-md px-2.5 py-1 sm:px-3 sm:py-1 text-xs sm:text-sm shadow-sm"
              style={{ 
                backgroundColor: "rgba(156, 163, 175, 0.95)",
          ...
          <div className="space-y-1.5 sm:space-y-2">
            <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
              <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center bg-gray-100 dark:bg-gray-700 flex-shrink-0">
                <MapPin className="w-2.5 h-2.5 sm:w-3 sm:h-3" style={{ color: "#0A4ABF" }} />
              </div>
              <span className="truncate">{product.location}{product.area ? `, ${product.area}` : ''}</span>
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
              <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center bg-gray-100 dark:bg-gray-700 flex-shrink-0">
                <User className="w-2.5 h-2.5 sm:w-3 sm:h-3" style={{ color: "#0A4ABF" }} />
              </div>
        ...
        <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-gray-200 dark:border-gray-700 mt-auto">
          <div className="flex-1 min-w-0 mr-2">
            <div className="flex flex-wrap items-baseline gap-1">
              ...
              <span className="text-sm sm:text-base text-gray-600 dark:text-gray-400">JOD</span>
            </div>
          </div>
          <Button
            size="sm"
            className="hover:opacity-90 transition-all duration-300 shadow-md hover:shadow-lg px-3 sm:px-4 md:px-6 text-xs sm:text-sm flex-shrink-0"
            style={{ backgroundColor: "#0A4ABF", color: "white" }}
            onClick={(e) => {
              e.stopPropagation();
              onProductClick?.(product.id);
