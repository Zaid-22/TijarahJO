...
export function LoginPage({ onLogin, onContinueAsGuest }: LoginPageProps) {
  ...
  const validatePhone = (value: string): string => {
    // Phone is optional - only validate if user enters something
    if (!value.trim()) return ""; // Allow empty phone
  ...
  const isFormValid = (): boolean => {
    if (isSignUp) {
      return (
        ...
        (phone.trim() === "" || !validatePhone(phone)) // Valid if empty OR valid phone format
      );
    } else {
                ...
                <div className="relative">
                  ...
                  >
                    <User
                      ...
                      style={{
                        color:
                          usernameFocused || username ? "#0A4ABF" : "#9CA3AF",
                      }}
                    />
                  </div>
                  <Input
