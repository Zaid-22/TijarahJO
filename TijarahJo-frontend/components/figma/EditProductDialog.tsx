...
export function EditProductDialog({
  ...
  const locations = ["Amman", "Irbid", "Zarqa", "Aqaba", "Madaba", "Karak", "Mafraq", "Tafilah"];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (priceValue < 0.01) {
      alert(language === "ar" ? "السعر يجب أن يكون 0.01 دينار على الأقل" : "Price must be at least 0.01 JOD");
      return;
    }
    ...
    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 mt-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="edit-name" className={isRTL ? "text-right block" : ""}>
        ...
        {/* Price */}
        <div className="space-y-2">
          <Label htmlFor="edit-price" className={isRTL ? "text-right block" : ""}>
        ...
        {/* Category */}
        <div className="space-y-2">
          <Label htmlFor="edit-category" className={isRTL ? "text-right block" : ""}>
            {language === "ar" ? "الفئة" : "Category"} *
          </Label>
          <Select value={category} onValueChange={setCategory} required>
            <SelectTrigger id="edit-category" className={isRTL ? "text-right" : ""}>
              <SelectValue
                placeholder={
        ...
        {/* City */}
        <div className="space-y-2">
          <Label htmlFor="edit-location" className={isRTL ? "text-right block" : ""}>
            {language === "ar" ? "المدينة" : "City"} *
          </Label>
          <Select value={location} onValueChange={setLocation} required>
            <SelectTrigger id="edit-location" className={isRTL ? "text-right" : ""}>
              <SelectValue
                placeholder={
        ...
        {/* Area */}
        <div className="space-y-2">
          <Label htmlFor="edit-area" className={isRTL ? "text-right block" : ""}>
          ...
          <Input
            ...
            placeholder={
              language === "ar" ? "مثال: عمان" : "e.g. Amman"
            }
            className={isRTL ? "text-right" : ""}
          />
        </div>
        ...
        <div className="space-y-2">
          <Label htmlFor="edit-description" className={isRTL ? "text-right block" : ""}>
            {language === "ar" ? "الو��ف (اختياري)" : "Description (Optional)"}
          </Label>
          <Textarea
            id="edit-description"
            value={description}
