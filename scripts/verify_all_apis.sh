...
if [ "$db_preflight_code" != "200" ]; then
  abort_verification \
    ...
    "Expected $BASE_URL/api/v1/categories to return 200, got $db_preflight_code. body=$db_preflight_body"
fi
if ! printf "%s" "$db_preflight_body" | jq -e 'type=="array"' >/dev/null 2>&1; then
  abort_verification \
    "preflight.database.unhealthy" \
    "Expected /api/v1/categories response to be a JSON array. body=$db_preflight_body"
fi
...
# 1. Location lookups (City/Area IDs)
location_city_id=""
location_area_id=""

require_api "locations.cities" "GET" "/api/v1/cities" "200"
assert_jq_or_abort "locations.cities.array" 'type=="array"'
location_city_id="$(printf "%s" "$LAST_BODY" | jq -r '.[0].CityId // .[0].cityId // empty')"

if is_positive_int "$location_city_id"; then
  require_api "locations.areas.bycity" "GET" "/api/v1/cities/$location_city_id/areas" "200"
  assert_jq_or_abort "locations.areas.bycity.array" 'type=="array"'
  location_area_id="$(printf "%s" "$LAST_BODY" | jq -r '.[0].AreaId // .[0].areaId // empty')"
else
  log_skip "locations.areas.bycity (no cities available in seed data)"
fi
...
invalid_location_email="apitest_${ts}_invalid_location@example.com"
...
require_api "auth.me.user1" "GET" "/api/v1/auth/me" "200" "" "$token1"
if [ -z "$user1_id" ]; then
...
require_api "auth.me.user2" "GET" "/api/v1/auth/me" "200" "" "$token2"
if [ -z "$user2_id" ]; then
...
if [ -n "$ADMIN_TOKEN" ]; then
  call_api "users.all.admin" "GET" "/api/v1/users" "200" "" "$ADMIN_TOKEN"
  assert_jq "users.all.admin.nonempty" 'type=="array" and length>0'
else
  call_api "users.all.restricted" "GET" "/api/v1/users" "401,403"
fi

require_api "users.byid" "GET" "/api/v1/users/$user1_id" "200"
require_api "categories.all" "GET" "/api/v1/categories" "200"
...
require_api "categories.byid" "GET" "/api/v1/categories/$first_category_id" "200"
...
assert_jq_or_abort "posts.all.gone.contract" '((.status // 0)==404) or ((.status // 0)==410) or ((.code // "")=="POSTS_LEGACY_ENDPOINT_REMOVED")'

require_api "posts.pagination.gone" "GET" "/api/v1/posts/pagination?pageNumber=1&rowsPerPage=10&includeDeleted=false" "404,410"
assert_jq_or_abort "posts.pagination.gone.contract" '((.status // 0)==404) or ((.status // 0)==410) or ((.code // "")=="POSTS_LEGACY_ENDPOINT_REMOVED")'

require_api "posts.feed.page1" "GET" "/api/v1/posts/feed?page=1&limit=10&includeDeleted=false" "200"
...
if is_positive_int "$first_post_id"; then
  require_api "posts.byid" "GET" "/api/v1/posts/$first_post_id" "200"
  require_api "posts.exists" "GET" "/api/v1/posts/Exists/$first_post_id" "200"
  assert_jq_or_abort "posts.exists.true" '. == true'
else
  log_skip "posts.byid / posts.exists (no posts returned)"
fi

require_api "posts.user.filter" "GET" "/api/v1/posts/user/1" "200"
assert_jq "posts.user.filter.array" 'type=="array"'
require_api "posts.category.filter" "GET" "/api/v1/posts/category/$first_category_id" "200"
assert_jq "posts.category.filter.array" 'type=="array"'

require_api "search.basic" "GET" "/api/v1/search?query=api&page=1&limit=10" "200"
assert_jq_or_abort "search.basic.shape" '.success==true and (.posts|type=="array") and (.pagination.currentPage >= 1)'

require_api "sellers.top" "GET" "/api/v1/sellers/top" "200"
...
if is_positive_int "$first_image_id"; then
  require_api "postimages.byid" "GET" "/api/v1/post-images/$first_image_id" "200,404"
  require_api "postimages.exists" "GET" "/api/v1/post-images/Exists/$first_image_id" "200"
else
  log_skip "postimages.byid / postimages.exists (no images returned)"
fi

require_api "reviews.get.user1" "GET" "/api/v1/reviews/user/$user1_id" "200"
...
if [ -n "$ADMIN_TOKEN" ]; then
  require_api "categories.create" "POST" "/api/v1/categories" "201" "$new_category_payload" "$ADMIN_TOKEN"
  ...
  if ! is_positive_int "$new_category_id"; then
    ...
    require_api "categories.get.new" "GET" "/api/v1/categories/$new_category_id" "200"
    ...
    require_api "categories.update" "PUT" "/api/v1/categories/$new_category_id" "200" "$updated_category_payload" "$ADMIN_TOKEN"
    ...
    require_api "categories.delete" "DELETE" "/api/v1/categories/$new_category_id" "200" "" "$ADMIN_TOKEN"
  fi
else
  require_api "categories.create.restricted" "POST" "/api/v1/categories" "401,403" "$new_category_payload" "$token1"
fi
...
if [ -n "$ADMIN_TOKEN" ]; then
  require_api "roles.create" "POST" "/api/v1/roles" "201" "$new_role_payload" "$ADMIN_TOKEN"
  ...
  if ! is_positive_int "$new_role_id"; then
    ...
    require_api "roles.update" "PUT" "/api/v1/roles/$new_role_id" "200" "$updated_role_payload" "$ADMIN_TOKEN"
    require_api "roles.delete" "DELETE" "/api/v1/roles/$new_role_id" "200" "" "$ADMIN_TOKEN"
  fi
else
  require_api "roles.create.restricted" "POST" "/api/v1/roles" "401,403" "$new_role_payload" "$token1"
fi
...
if is_positive_int "$location_city_id" && is_positive_int "$location_area_id"; then
  new_post_payload="$(jq -nc --argjson uid "$user2_id" --argjson cid "$first_category_id" --argjson city "$location_city_id" --argjson area "$location_area_id" --arg t "$now_iso" '{PostID:null,UserID:$uid,CategoryID:$cid,PostTitle:"API test post",PostDescription:"Created by verify_all_apis.sh",Price:123.45,Status:0,CreatedAt:$t,IsDeleted:false,Views:0,CityId:$city,AreaId:$area,Images:["https://example.com/a.jpg"]}')"
elif is_positive_int "$location_city_id"; then
  new_post_payload="$(jq -nc --argjson uid "$user2_id" --argjson cid "$first_category_id" --argjson city "$location_city_id" --arg t "$now_iso" '{PostID:null,UserID:$uid,CategoryID:$cid,PostTitle:"API test post",PostDescription:"Created by verify_all_apis.sh",Price:123.45,Status:0,CreatedAt:$t,IsDeleted:false,Views:0,CityId:$city,Images:["https://example.com/a.jpg"]}')"
else
...
if ! is_positive_int "$new_post_id"; then
  ...
  require_api "posts.get.new" "GET" "/api/v1/posts/$new_post_id" "200"
  ...
  require_api "posts.user.filter.newuser" "GET" "/api/v1/posts/user/$user2_id" "200"
  assert_jq "posts.user.filter.newuser.nonempty" 'type=="array" and length>0'
  require_api "posts.category.filter.newpost" "GET" "/api/v1/posts/category/$first_category_id" "200"
  ...
  require_api "favorites.add.user1.newpost" "POST" "/api/v1/favorites" "200" "$favorite_payload" "$token1"
  require_api "favorites.get.user1.after.add" "GET" "/api/v1/favorites" "200" "" "$token1"
  assert_jq "favorites.get.user1.after.add.nonempty" '.success==true and (.favorites|type=="array") and ((.favorites|length) >= 1)'
  require_api "favorites.remove.user1.newpost" "DELETE" "/api/v1/favorites/$new_post_id" "200" "" "$token1"
  require_api "favorites.get.user1.after.remove" "GET" "/api/v1/favorites" "200" "" "$token1"
  assert_jq "favorites.get.user1.after.remove.shape" '.success==true and (.favorites|type=="array")'

  require_api "search.newpost.query" "GET" "/api/v1/search?query=API%20test%20post&page=1&limit=20" "200"
  ...
  require_api "postimages.create" "POST" "/api/v1/post-images" "201" "$new_image_payload" "$token2"
  ...
  if ! is_positive_int "$new_image_id"; then
    ...
    require_api "postimages.update" "PUT" "/api/v1/post-images/$new_image_id" "200" "$update_image_payload" "$token2"
    require_api "postimages.get.new" "GET" "/api/v1/post-images/$new_image_id" "200"
    require_api "postimages.delete" "DELETE" "/api/v1/post-images/$new_image_id" "200" "" "$token2"
  fi
fi
...
require_api "reviews.create" "POST" "/api/v1/reviews" "201,403" "$review_payload" "$token2"
require_api "reviews.get.user1.after" "GET" "/api/v1/reviews/user/$user1_id" "200"
...
require_api "chat.history.bootstrap.user2-user1" "GET" "/api/v1/chat/history/$user1_id" "200" "" "$token2"
assert_jq "chat.history.bootstrap.user2-user1.array" 'type=="array"'
conversation_id="$(printf "%s" "$LAST_BODY" | jq -r '.[0].ConversationId // empty')"

message_payload="$(jq -nc --argjson rid "$user1_id" --argjson pid "$chat_post_id" '{ReceiverId:$rid,PostId:$pid,Content:"Hello from API test"}')"
require_api "chat.send.receiver" "POST" "/api/v1/chat/send" "200" "$message_payload" "$token2"

if is_positive_int "$conversation_id"; then
  conversation_message_payload="$(jq -nc --argjson cid "$conversation_id" '{ConversationId:$cid,Content:"Hello again via conversation id"}')"
  require_api "chat.send.conversation" "POST" "/api/v1/chat/send" "200" "$conversation_message_payload" "$token2"
fi
...
require_api "users.exists.auth" "GET" "/api/v1/users/Exists/$user1_id" "200" "" "$token1"
assert_jq "users.exists.auth.true" '. == true'

require_api "users.get.user2.before.update" "GET" "/api/v1/users/$user2_id" "200" "" "$token2"
user2_model="$(printf "%s" "$LAST_BODY" | jq '.FirstName="APIUpdated"')"
require_api "users.update.user2" "PUT" "/api/v1/users/$user2_id" "200" "$user2_model" "$token2"
