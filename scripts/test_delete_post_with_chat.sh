...
call_api() {
  ...
  local -a args
  args=(-sS -o "$body_file" -w "%{http_code}" -X "$method" "$API_ROOT$path" -H "Content-Type: application/json")
...
call_api "auth.signup.user1" "POST" "/auth/signup" "201" "$signup1_payload"
...
call_api "auth.signup.user2" "POST" "/auth/signup" "201" "$signup2_payload"
...
call_api "categories.all" "GET" "/categories" "200"
...
if [ -n "$user2_id" ] && [ -n "$first_category_id" ]; then
  ...
  call_api "posts.create" "POST" "/posts" "201" "$new_post_payload" "$token2"
...
if [ -n "$new_post_id" ] && [ -n "$user1_id" ] && [ -n "$user2_id" ]; then
  ...
  call_api "chat.send" "POST" "/chat/send" "200" "$message_payload" "$token1"
fi

# 5) Delete the post and ensure it succeeds.
if [ -n "$new_post_id" ]; then
  call_api "posts.delete" "DELETE" "/posts/$new_post_id" "200" "" "$token2"
  call_api "posts.exists.after.delete" "GET" "/posts/Exists/$new_post_id" "200"
